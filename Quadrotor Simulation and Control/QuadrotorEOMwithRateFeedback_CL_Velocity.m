function [var_dot] = QuadrotorEOMwithRateFeedback_CL(t, var, g, m, I, nu, mu,lat,long,type)
...
L_c = Gc(1); %d/sqrt(2)*(-motor_forces(1)-motor_forces(2)+motor_forces(3)+motor_forces(4));
M_c = Gc(2); %d/sqrt(2)*(motor_forces(1)-motor_forces(2)-motor_forces(3)+motor_forces(4));
N_c = Gc(3); %d/sqrt(2)*(motor_forces(1)-motor_forces(2)+motor_forces(3)-motor_forces(4));
%% Position Data 
